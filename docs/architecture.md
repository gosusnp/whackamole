...
- `internal/cli/static/`: Embedded static assets for the Web UI.
...
- `internal/version.go`: Application version definition.
...
2. **Web UI & API Layer (`internal/cli/ui.go`)**: When the `ui` command is run, `whackamole` starts an embedded web server.
    - **Static Assets**: Serves the Preact-based frontend from `internal/cli/static`.
    - **API Endpoints**: Provides a JSON API (`/api/...`) that the frontend uses to interact with the Store layer.
3. **Store Layer (`internal/db`)**: Manages all database interactions. Following the **Fat Store** approach, this layer is responsible for:
...
4. **Model Layer (`internal/types`)**: Defines the structure of entities (like `Project`) used throughout the system.
...
- **JSON Casing**: We use `camelCase` for JSON tags (e.g., `json:"id"`, `json:"projectId"`) to maintain consistency across API boundaries and standard JSON practices.
